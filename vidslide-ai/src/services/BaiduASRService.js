...
class BaiduASRService {
  constructor(options = {}) {
    this.appId = options.appId || process.env.BAIDU_ASR_APP_ID;
    this.apiKey = options.apiKey || process.env.BAIDU_ASR_API_KEY;
    this.secretKey = options.secretKey || process.env.BAIDU_ASR_SECRET_KEY;
  ...
  async transcribe(videoPath) {
    try {
      ...
      if (response.data.err_no === 0) {
        ...
        throw new Error(`ASR识别失败: ${response.data.err_msg}`);
      }
  ...
  async transcribeLong(videoPath, segmentDuration = 60) {
    try {
      ...
      for (let i = 0; i < segments; i++) {
        ...
        const segmentTranscript = await this.transcribeSegment(segmentPath);
  ...
  /**
   * 识别音频片段
   * @param {string} segmentPath - 片段路径
   * @returns {Promise<string>} 识别文本
   */
  async transcribeSegment(segmentPath) {
    try {
      ...
      if (response.data.err_no === 0) {
        ...
        throw new Error(`片段识别失败: ${response.data.err_msg}`);
      }

    } catch (error) {
      throw new Error(`识别音频片段失败: ${error.message}`);
    }
  }
}

...
class BaiduNLPService {
  ...
  async getAccessToken() {
    ...
    try {
      ...
      const baseUrl = isBrowser
        ? '/api/baidu'  // 浏览器环境使用Vite代理
        : 'https://aip.baidubce.com'  // Node.js环境使用完整URL

      const url = `${baseUrl}/oauth/2.0/token?grant_type=client_credentials&client_id=${apiKey}&client_secret=${secretKey}`
  ...
  async extractKeywords(text, num = 15) {
    ...
    try {
      ...
      // 百度NLP关键词提取API
      const url = `${baseUrl}/rpc/2.0/nlp/v1/keyword?access_token=${token}`
    ...
    for (const word of sortedDomainWords) {
      ...
      if (matches && matches.length > 0) {
        ...
        const normalizedWord =
          word.toUpperCase().includes('AD') || word.toUpperCase().includes('IP')
            ? word
                .replace(/ad$/i, 'AD')
                .replace(/ip$/i, 'IP')
                .replace(/^做ip$/i, '做IP')
    ...
    for (const item of wordScores) {
      ...
      for (const used of usedWords) {
        ...
        if (item.text.includes(used)) {
          if (
            item.isDomain ||
            item.score > filteredKeywords.find(k => k.text === used)?.score * 1.5
          ) {

...
class DoubaoImageService {
  ...
  async initialize() {
    ...
    try {
      ...
      // 导入其他服务
      const PromptOptimizerModule = await import('./PromptOptimizer.js');
      this.promptOptimizer = new PromptOptimizerModule.default();

      // AdvancedPromptGenerator 是可选的
      try {
        const AdvancedPromptGeneratorModule = await import('./AdvancedPromptGenerator.js');
        this.advancedPromptGenerator = new AdvancedPromptGeneratorModule.default();
      } catch (error) {
        console.log('⚠️ AdvancedPromptGenerator 不可用，使用基础提示词优化');
        this.advancedPromptGenerator = null;
      }
   ...
   * @param {boolean} context.useCache - 是否使用缓存（默认true）
   * @param {boolean} context.forceRefresh - 是否强制刷新（默认false）
   * @param {string} context.videoId - 视频ID（用于缓存键）
   * @param {string} context.sessionId - 会话ID（用于缓存键）
  ...
  async generateImage(keyword, context = {}) {
    ...
    try {
      ...
      const cacheKey = this.getCacheKey(keyword, context); // 生成缓存键
      ...
      const useAdvanced = context.useAdvancedPrompt !== false && this.advancedPromptGenerator; // 默认使用高级生成器（如果可用）
  ...
  clearCache() {
    ...
    try {
      ...
      console.log(`[DoubaoImageService] 缓存已清空，删除了 ${count} 个文件`);
      return count;
    } catch (error) {
  ...
  cleanExpiredCache(maxAge = 24 * 60 * 60 * 1000) {
    if (this.isBrowser || !this.fs || !this.path) {
      console.log('[DoubaoImageService] 浏览器环境不支持清理缓存');
      return 0;
    }
    ...
    try {
      ...
      console.error(`[DoubaoImageService] 清理过期缓存失败:`, error.message);
      return 0;
    }
  }
  ...
  getCacheInfo() {
    ...
    try {
      ...
      console.error(`[DoubaoImageService] 获取缓存信息失败:`, error.message);
      return { count: 0, size: 0 };
    }
  }

/**
 * 千帆平台IAM签名认证测试脚本
 * 测试基于BCE AK/SK签名的认证方式
 */

import QianfanIAMService from '../src/services/QianfanIAMService.js';

console.log('\n' + '='.repeat(60));
console.log('🔐 千帆平台IAM签名认证测试');
console.log('='.repeat(60));

async function testQianfanIAM() {
  const service = new QianfanIAMService();
...
\`\`\``;
    ...
    // 最终结果
    console.log('\n' + '='.repeat(60));
    console.log('✅ 所有测试通过！千帆IAM签名认证工作正常');
    console.log('='.repeat(60));
    console.log('\n📌 下一步: 更新ContentAnalyst使用新的IAM服务');
    console.log('   文件: src/agents/executors/ContentAnalyst.js');
    console.log('='.repeat(60));
    ...
    console.log('\n💡 可能的原因:');
    console.log('   1. BCE签名算法实现有误');
    console.log('   2. Access Key或Secret Key错误');
    console.log('   3. 应用未开通ERNIE-3.5-8K服务');
    console.log('   4. 请求头或路径格式不正确');
    console.log('   5. 网络连接问题');

    console.log('\n🔧 调试建议:');
    console.log('   1. 检查签名算法是否符合BCE IAM规范');
    console.log('   2. 验证时间戳格式是否正确');
    console.log('   3. 确认请求路径和参数编码');

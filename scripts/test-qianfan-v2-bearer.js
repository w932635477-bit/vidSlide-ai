/**
 * 千帆平台V2协议Bearer Token认证测试
 * 测试正确的v2协议认证方式
 */

import QianfanV2BearerService from '../src/services/QianfanV2BearerService.js';

console.log('\n' + '='.repeat(60));
console.log('🎯 千帆平台V2协议Bearer Token认证测试');
console.log('='.repeat(60));

async function testQianfanV2Bearer() {
  const service = new QianfanV2BearerService();
...
\`\`\``;
    ...
    // 最终结果
    console.log('\n' + '='.repeat(60));
    console.log('✅ 所有测试通过！千帆V2协议Bearer Token认证工作正常');
    console.log('='.repeat(60));
    console.log('\n📌 下一步: 更新ContentAnalyst使用新的服务');
    console.log('   文件: src/agents/executors/ContentAnalyst.js');
    console.log('   替换: QianfanWenxinService → QianfanV2BearerService');
    console.log('='.repeat(60));
    ...
    console.log('\n💡 可能的原因:');
    console.log('   1. Bearer Token格式错误');
    console.log('   2. API endpoint不正确');

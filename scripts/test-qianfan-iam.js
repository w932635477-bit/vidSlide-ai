/**
 * 千帆平台IAM认证测试脚本
 * 验证新的QianfanWenxinService是否能够成功调用文心一言API
 */

import QianfanWenxinService from '../src/services/QianfanWenxinService.js';

console.log('\n' + '='.repeat(60));
console.log('🔑 千帆平台IAM认证测试');
console.log('='.repeat(60));

async function testQianfanIAM() {
  const service = new QianfanWenxinService();
...
\`\`\``;
    ...
    // 最终结果
    console.log('\n' + '='.repeat(60));
    console.log('✅ 所有测试通过！千帆IAM认证工作正常');
    console.log('='.repeat(60));
    console.log('\n📌 下一步: 运行完整ContentAnalyst测试');
    ...
    console.log('   2. 应用未开通ERNIE-3.5-8K服务');
    console.log('   3. 账号未实名认证');
    console.log('   4. 免费额度已用完');
    console.log('   5. 网络连接问题');

    console.log('\n🔧 解决方案:');
    console.log('   1. 检查src/services/QianfanWenxinService.js中的密钥配置');
    console.log('   2. 访问千帆控制台确认服务状态');
    console.log('   3. 查看应用权限和额度');
